...
class RescheduleActionRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            // A date input is rendered only for the cadences where a day
            // means something — `weekly`, `fortnightly`, `monthly` and
            // `once`, per `recurrences.ts` — so most saves carry no date,
            // and a save that carries none takes the derived-anchor path
            // this endpoint has always taken. Not added to
            // withValidator()'s "at least one field" set: `kind` already
            // marks that a schedule was submitted, and the form always
            // sends `kind` alongside `date`.
