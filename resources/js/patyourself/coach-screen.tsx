...
import type { CoachAction, Intention, Outcome, ThreadItem } from './data';
...
function buildInitialThread(intentions: Intention[]): ThreadItem[] {
    const items: ThreadItem[] = [
        ...getCoachResponse('morning_greet', { intentions }),
    ];

    for (const it of intentions) {
        items.push({ kind: 'action-card', intentionId: it.id });
    }
...
function initState(): State {
    const intentions = SEED_INTENTIONS.map((i) => ({
        ...i,
        log: i.log.slice(),
        strategies: i.strategies.slice(),
    }));
...
function findLastDiscussed(state: State): string | null {
    for (let i = state.thread.length - 1; i >= 0; i--) {
        ...
        if (t.kind === 'action-card' || t.kind === 'reason-chips') {
            return t.intentionId;
        }
    }
...
function reducer(state: State, action: Action): State {
    switch (action.type) {
        case 'user_msg':
            return {
                ...state,
                thread: [...state.thread, { kind: 'user', text: action.text }],
            };
        case 'coach_msg':
        ...
        case 'log_outcome': {
            ...
            const intentions = state.intentions.map((it) => {
                ...
                return {
                    ...it,
                    log: [...filtered, { date: t, outcome: action.outcome }],
                };
            });
                ...
                const strategies = it.strategies.map((s) =>
                    s.status === 'active'
                        ? {
                              ...
                              reason: s.reason || 'Re-strategized',
                          }
                        : s,
                );
                const nextV =
                    (strategies[strategies.length - 1]?.version || 0) + 1;
                strategies.push({
                    version: nextV,
                    action: action.payload,
                    status: 'active',
                    reason: 'Iteration on previous version.',
                    createdAt: ymd(today),
                });
...
export function CoachScreen() {
    ...
    function send(text: string) {
        ...
        setTimeout(
            () =>
                dispatch({
                    type: 'coach_msg',
                    items: getCoachResponse('typed_general'),
                }),
            350,
        );
    }
    function logCard(intentionId: string, outcome: Outcome) {
        ...
        const intent =
            outcome === 'skipped'
                ? 'logged_skip_ask_reason'
                : outcome === 'urge-survived'
                  ? 'logged_urge'
                  : 'logged_done';
        setTimeout(
            () =>
                dispatch({
                    type: 'coach_msg',
                    items: getCoachResponse(intent, { intention: it }),
                }),
            300,
        );
    }
    function pickReason(intentionId: string, reason: string) {
        ...
        setTimeout(() => {
            ...
            const proposalText = r.includes('tired')
                ? 'Move action to evenings'
                : r.includes('time')
                  ? 'Halve the action (under 2 minutes)'
                  : r.includes('forgot')
                    ? 'Pair with brushing teeth'
                    : 'Try a 30-second version';
            dispatch({
                type: 'coach_msg',
                items: getCoachResponse('reason_received_propose_strategy', {
                    intention: it,
                    reason,
                    proposalText,
                }),
        ...
        if (action.intent === 'accept_strategy') {
            ...
            setTimeout(
                () =>
                    dispatch({
                        type: 'coach_msg',
                        items: getCoachResponse('strategy_accepted'),
                    }),
                300,
            );
                ...
                items: [
                    {
                        kind: 'coach',
                        sig: 'stack',
                        html: 'Try this: <em>after lifting, do five minutes of mobility on the mat.</em> Same cue, new tail.',
                    },
                ],
            });
            ...
            dispatch({
                type: 'coach_msg',
                items: [{ kind: 'coach', html: "Got it — I won't push." }],
            });
                ...
                items: [
                    {
                        kind: 'coach',
                        sig: 'setup',
                        html: "Let's build a fresh loop — the setup flow opens here soon.",
                    },
                ],
            });
        }
        ...
        <div className="screen-chat">
            <div className="coach-head">
                ...
                <div className="coach-progress">
                    <span className="coach-progress__label">
                        <b>{doneCount}</b> of {total} logged
                    </span>
                    <span
                        className="coach-progress__dots"
                        role="img"
                        aria-label={`${doneCount} of ${total} loops logged today`}
                    >
                        {Array.from({ length: total }).map((_, i) => (
                            <i
                                key={i}
                                className={i < doneCount ? 'done' : ''}
                                aria-hidden="true"
                            />
                        ))}
                    </span>
                        ...
                        <div className="chat-inner">
                            ...
                            {thread.map((item, idx) => {
                                ...
                                if (item.kind === 'action-card') {
                                    const it = intentions.find(
                                        (i) => i.id === item.intentionId,
                                    );
