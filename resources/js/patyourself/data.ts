...
export const SEED_INTENTIONS: Intention[] = [
  {
    ...
    strategies: [
      { version: 1, action: 'Put on shoes and walk out the door', status: 'failed', reason: "Too tired — couldn't get past the bed.", createdAt: daysAgo(21) },
      { version: 2, action: 'Set alarm 30 min earlier (6:30)', status: 'failed', reason: 'Snoozed through it three days in a row.', createdAt: daysAgo(14) },
      { version: 3, action: 'Lay out gym clothes on the chair the night before', status: 'active', reason: 'Removes the morning decision.', createdAt: daysAgo(7) },
    ],
    log: [
  ...
  },
  {
    ...
    strategies: [
      { version: 1, action: 'Drink a glass of water after lunch instead', status: 'active', reason: 'Replace the ritual, not the moment.', createdAt: daysAgo(10) },
    ],
    log: [
  ...
  },
  {
    ...
    strategies: [
      { version: 1, action: 'Put phone across the room', status: 'active', reason: 'Forces me to stand up to turn it off.', createdAt: daysAgo(3) },
    ],
    log: [
      { date: daysAgo(2), outcome: 'done' },
...
export function activeStrategy(it: Intention): Strategy | undefined {
  return it.strategies.slice().reverse().find((s) => s.status === 'active');
}
export function currentStreak(it: Intention): number {
  let streak = 0;
  for (let i = it.log.length - 1; i >= 0; i--) {
    const o = it.log[i].outcome;
    if (o === 'done' || o === 'urge-survived') streak++;
    else break;
  }
  return streak;
}
export function patternFlag(it: Intention): 'stack-ready' | 'needs-restrategy' | null {
  if (currentStreak(it) >= 5) return 'stack-ready';
  const recent = it.log.slice(-5).filter((l) => l.outcome === 'skipped');
  if (recent.length >= 3) return 'needs-restrategy';
  return null;
}
...
export function getCoachResponse(intent: string, ctx: CoachCtx = {}): ThreadItem[] {
  const it = ctx.intention;
  switch (intent) {
    case 'morning_greet':
      return [{ kind: 'coach', sig: 'good morning', html: 'Three loops today. Want to go one by one, or just give me a quick all-clear?' }];
    ...
    case 'logged_done': {
      if (!it) return [];
      const streak = currentStreak({ ...it, log: [...it.log, { date: ymd(today), outcome: 'done' }] });
      if (streak >= 5) {
        return [{
          kind: 'coach', sig: 'pattern noticed',
          ...
          actions: [
            { label: 'Yes, propose one', intent: 'propose_stack' },
            { label: 'Not yet', intent: 'dismiss' },
          ],
        }];
      }
      return [{ kind: 'coach', sig: 'logged', html: `Logged. Nice — that's <b>${streak}</b> ${streak === 1 ? 'day' : 'days'} running.` }];
    }

    case 'logged_urge':
      if (!it) return [];
      return [{ kind: 'coach', sig: 'logged', html: `Surviving the urge counts twice. That's the real work on <em>${it.title.toLowerCase()}</em>.` }];

    case 'logged_skip_ask_reason':
      if (!it) return [];
      return [
        { kind: 'coach', sig: 'no blame', html: 'Okay. What got in the way?' },
    ...
    case 'reason_received_propose_strategy': {
      ...
      if (r.includes('tired')) proposal = 'Tiredness keeps winning the morning. Want to flip this to evenings instead?';
      if (r.includes('time')) proposal = 'Time is the cost. Want to halve the action — make it doable in under 2 minutes?';
      if (r.includes('forgot')) proposal = 'Pair it with something you already do — right after brushing teeth, for example.';
      if (r.includes('feel')) proposal = "That's worth naming. Sometimes the strategy is fine and the moment isn't — want to schedule a check-in tomorrow at the same cue?";
      return [{
        kind: 'coach', sig: 'iterating',
        html: `Got it — <em>"${ctx.reason}"</em>. ${proposal}`,
        actions: [
          { label: 'Try that', intent: 'accept_strategy', payload: ctx.proposalText || 'Try evenings instead' },
    ...
    case 'strategy_accepted':
      return [{ kind: 'coach', sig: 'logged change', html: "New strategy added. We'll track it from today. No pressure — first try is always rough." }];

    case 'typed_general':
      return [{ kind: 'coach', sig: 'reading', html: 'Heard. Want me to pull up a specific loop, or keep talking?' }];
...
export const REASON_CHIPS = ['Too tired', 'No time', 'Forgot', "Didn't feel like it", 'Other'];
