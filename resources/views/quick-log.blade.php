        ...
        <div class="flex min-h-screen items-center justify-center p-6">
            <div class="w-full max-w-sm rounded-xl border border-border bg-card p-6 text-center shadow-sm">
                <p class="text-xs font-medium tracking-wide text-muted-foreground uppercase">
                    {{ $alreadyLogged ? 'No change made' : 'Recorded' }}
                </p>

                <h1 class="mt-1 font-display text-lg font-semibold text-foreground">
                    {{ $title }}
                </h1>

                <p class="mt-3 text-sm text-muted-foreground">
                    @if ($alreadyLogged)
                        This occasion was already logged as
                        <span class="font-medium text-foreground">{{ ucfirst($outcome) }}</span>.
                    @else
                        Marked <span class="font-medium text-foreground">{{ ucfirst($outcome) }}</span>.
                    @endif
                </p>

                <p class="mt-4 text-xs text-muted-foreground">
                    You can change this in the app.
                </p>
